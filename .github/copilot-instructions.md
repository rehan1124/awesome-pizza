...
- **Structure & tags:** smoke tests under `tests/smoke-tests`, regression under
  ...
  `@smoke` or `@regression`.
- **Reporting:** decorate every public async page-object method with `@Step()` (imported
  from `@utils/step`) so the HTML report shows `ClassName.methodName` with the low-level
  steps nested inside. Call it with parentheses (`@Step()`, not `@Step`).

## Required checks after editing (every time)
There are **no local git hooks** — nothing runs automatically on save or commit. After
creating or editing any file under `playwright/`, run the quality gate yourself from the
`playwright/` folder and make it pass before finishing. These are the exact checks CI
enforces in its `quality` job:
